                    ...
                    <form method="post" action="<?= $view->action('add_localized_stack') ?>">
                        ...
                        <p>
                            <button class="btn btn-primary"
                                    type="submit"><?= $isGlobalArea ? t('Create localized global area version') : t('Create localized stack version') ?></button>
                            <br/>
                        </p>
                    </form>
                    <?php
                ...
                <div class="collapse navbar-collapse">
                    <ul class="navbar-nav mr-auto">
                        ...
                        if ($areaPermissions->canAddBlocks()) {
                            ...
                            <li class="nav-item dropdown">
                                <a class="dropdown-toggle nav-link" data-toggle="dropdown" href="#"><?= t('Add') ?>
                                    <span class="caret"></span></a>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item dialog-launch" dialog-modal="false" dialog-width="550"
                                           dialog-height="380" dialog-title="<?= t('Add') ?>"
                                           href="<?= URL::to('/ccm/system/dialogs/page/add_block_list') ?>?cID=<?= $stackToEdit->getCollectionID() ?>&arHandle=<?= STACKS_AREA_NAME ?>"><?= t('Add Block') ?></a>
                                    </li>
                                    <li><a class="dropdown-item dialog-launch" dialog-modal="false" dialog-width="550"
                                           dialog-height="380" dialog-title="<?= t('Paste From Clipboard') ?>"
                                           href="<?= URL::to('/ccm/system/dialogs/page/clipboard') ?>?cID=<?= $stackToEdit->getCollectionID() ?>&arHandle=<?= STACKS_AREA_NAME ?>"><?= t('Paste From Clipboard') ?></a>
                                    </li>
                                </ul>
                            </li>
                            <?php
                        ...
                        if ($cpc->canViewPageVersions()) {
                            ...
                            <li class="nav-item">
                                <a dialog-width="640" dialog-height="340"
                                   ...
                                   href="<?= URL::to('/ccm/system/panels/page/versions') ?>?cID=<?= $stackToEdit->getCollectionID() ?>">
                                    <?= t('Version History') ?>
                                </a>
                            </li>
                            <?php
                        ...
                        if (!$isGlobalArea && $cpcNeutral->canEditPageProperties()) {
                            ...
                            <li class="nav-item">
                                <a class="nav-link"
                                   href="<?= $view->action('rename', $neutralStack->getCollectionID()) ?>"><?= t('Rename') ?></a>
                            </li>
                            <?php
                        }
                        if ($cpc->canEditPagePermissions() && Config::get('concrete.permissions.model') == 'advanced') {
                            ...
                            <li class="nav-item">
                                <a dialog-width="580" class="nav-link dialog-launch"
                                   ...
                                   href="<?= URL::to('/ccm/system/panels/details/page/permissions?cID=' . $stackToEdit->getCollectionID()) ?>">
                                    <?= t('Permissions') ?>
                                </a>
                            </li>
                            <?php
                    ...
                    <?php if ($showApprovalButton) { ?>
                        <ul class="nav navbar-nav navbar-right">
                            <li id="ccm-stack-list-approve-button nav-item"
                                class="navbar-form"<?= $vo->isApprovedNow() ? ' style="display: none;"' : '' ?>>
                                <button class="btn btn-success"
                                        onclick="window.location.href='<?= URL::to('/dashboard/blocks/stacks', 'approve_stack', $stackToEdit->getCollectionID(), $token->generate('approve_stack')) ?>'"><?= $publishTitle ?></button>
                            </li>
                        </ul>
                    <?php } ?>
                </div>
        ...
        ?>
            <div style="display: none">
                <div id="ccm-dialog-delete-stack" class="ccm-ui">
                    <form method="post" class="form-stacked" style="padding-left: 0px"
                          action="<?= $view->action('delete_stack') ?>">
                    ...
                    <div class="dialog-buttons">
                        ...
                        <button class="btn btn-danger ml-auto"
                                onclick="$('#ccm-dialog-delete-stack form').submit()"><?= $deleteLabels['button'] ?></button>
                    </div>
                </div>
            </div>
        <?php
...
if ($showGlobalAreasFolder || !empty($stacks)) {
    ...
    <div class="table-responsive">
        <table class="ccm-search-results-table">
            ...
            <tr>
                <th></th>
                <th class="<?= $list->getSortClassName('cv.cvName') ?>"><a
                            href="<?= $list->getSortURL('cv.cvName') ?>"><?= t('Name') ?></a></th>
                <th class="<?= $list->getSortClassName('c.cDateAdded') ?>"><a
                            href="<?= $list->getSortURL('c.cDateAdded') ?>"><?= t('Date Added') ?></a></th>
                <th></th>
            </tr>
            </thead>
            <tbody>
            ...
            if ($showGlobalAreasFolder) {
                ?>
                <tr class="ccm-search-results-folder ccm-search-results-globalareafolder"
                    data-details-url="<?= $view->url('/dashboard/blocks/stacks', 'view_global_areas') ?>">
                ...
                <tr class="<?= $formatter->getSearchResultsClass() ?>"
                    ...
                    <td class="ccm-search-results-menu-launcher">
                        <?php if ($st->getCollectionTypeHandle() == STACK_CATEGORY_PAGE_TYPE) { ?>
                            <div class="dropdown">
                                <button class="btn btn-icon" data-boundary="viewport" type="button"
                                ...
                                <div class="dropdown-menu">
                                    <a data-action="rename" class="dropdown-item"
                                       href="<?= $view->action('rename', $st->getCollectionID()) ?>"><?= t('Rename Folder') ?></a>
                                    <a data-action="delete" class="dropdown-item"
                                       data-folder-id="<?= $st->getCollectionID() ?>"
                                       ref="javascript:void(0)"><?= t('Delete Folder') ?></a>
                                </div>
                            </div>
    ...
    <div style="display: none">
        <div id="ccm-dialog-delete-stackfolder" class="ccm-ui" title="<?= t('Delete Folder') ?>">
            <form method="post" class="form-stacked" style="padding-left: 0px"
                  action="<?= $view->action('delete_stackfolder') ?>">
            ...
            <div class="dialog-buttons">
                ...
                <button class="btn btn-danger ml-auto"
                        onclick="$('#ccm-dialog-delete-stackfolder form').submit()"><?= t('Delete Folder') ?></button>
            </div>
        </div>
    </div>
    ...
    <div class="ccm-dashboard-header-buttons">
        ...
        if ($controller->getAction() != 'view_global_areas') {
            ?>
            <div class="btn-group">
                <button data-dialog="add-stack" class="btn btn-secondary"><i
                            class="fa fa-bars"></i> <?= t('New Stack') ?></button>
                <button data-dialog="add-folder" class="btn btn-secondary"><i
                            class="fa fa-folder"></i> <?= t('New Folder') ?></button>
            </div>
            <?php
        }
        ?>
        ...
        <div id="ccm-dialog-add-folder" class="ccm-ui">
            ...
            <div class="dialog-buttons">
                <button class="btn btn-secondary mr-auto"
                        onclick="jQuery.fn.dialog.closeTop()"><?= t('Cancel') ?></button>
                <button class="btn btn-primary float-right"
                        onclick="$('#ccm-dialog-add-folder form').submit()"><?= t('Add Folder') ?></button>
            </div>
        </div>
    </div>
